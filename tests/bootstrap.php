...
if (! \function_exists('recursiveDirectoryRegexSearch')) {
    function recursiveDirectoryRegexSearch(string $path, string $regex): \Generator
    {
...
if (\is_dir($autoloadPath)) {
    ...
    foreach (\recursiveDirectoryRegexSearch($autoloadPath, '#^.+\.php$#iu') as $file) {
...
foreach ($phpVersions as $versionChecked) {
    ...
    if ($currentPHPVersion === $versionChecked) {
        ...
        if (\is_dir($phpVersionOnlyPath)) {
            ...
            foreach (\recursiveDirectoryRegexSearch($phpVersionOnlyPath, '#^.+\.php$#iu') as $file) {
                require_once $file->getPathname();
            }
        }
    }
    ...
    if (\is_dir($phpVersionPath)) {
        ...
        foreach (\recursiveDirectoryRegexSearch($phpVersionPath, '#^.+\.php$#iu') as $file) {
            require_once $file->getPathname();
        }
    }
}
